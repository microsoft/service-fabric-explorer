...
import { InfrastructureJobsComponent } from './infrastructure-tasks.component';
...
describe('InfrastructureJobsComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    dataServiceStub.restClient = ({
      getInfrastructureJobs(messageHandler?: IResponseMessageHandler): Observable<IRawInfrastructureJob[]> {
        return of([
        task,
        {
          Scope: {
    ...
    dataServiceStub.repairCollection = new InfrastructureJobCollection(dataServiceStub as DataService);
  ...
  fit('repair tasks in proper places', () => {
    expect(component.infrastructureJobCollection.infrastructureJobs.length).toBe(1);
    expect(component.infrastructureJobCollection.completedInfrastructureJobs.length).toBe(1);
    expect(component.sortedCompletedInfrastructureJobs.length).toBe(0);
    expect(component.sortedInfrastructureJobs.length).toBe(0);

    const newTask = new InfrastructureJob(TestBed.inject(DataService), task);

    component.sorted([newTask]);

    expect(component.sortedCompletedInfrastructureJobs.length).toBe(1);
    expect(component.sortedInfrastructureJobs.length).toBe(0);

    const newTask2 = new InfrastructureJob(TestBed.inject(DataService), {...task});
    newTask2.raw.TaskId = 'test';

    component.sorted([newTask2], false);

    expect(component.sortedCompletedInfrastructureJobs.length).toBe(1);
    expect(component.sortedInfrastructureJobs.length).toBe(1);
  });
});
