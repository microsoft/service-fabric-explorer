...
export interface IInfrastructureJobHistoryPhase {
    timestamp: string;
    phase: string;
    duration: string;
...
export interface IInfrastructureJobPhase {
    name: string;
    duration: string;
    durationMilliseconds: number;
    phases: IInfrastructureJobHistoryPhase[];
...
export class InfrastructureJob extends DataModelBase<IRawInfrastructureJob> {
    ...
    public history: IInfrastructureJobHistoryPhase[] = [];
    ...
    public historyPhases: IInfrastructureJobPhase[];
    ...
    */
    public get startTime(): Date {
        return new Date(this.raw.History.ExecutingUtcTimestamp === InfrastructureJob.NonStartedTimeStamp ? this.raw.History.CreatedUtcTimestamp :
    ...
    private parseHistory() {
        ...
        this.history = history.map((phase, index) => {
            ...
            if (index < (history.length - 1)) {
                ...
                if (nextPhase.timestamp !== InfrastructureJob.NonStartedTimeStamp) {
                ...
                } else if (phase.timestamp !== InfrastructureJob.NonStartedTimeStamp) {
            ...
            if (index === (history.length - 1)) {
                if (phase.timestamp !== InfrastructureJob.NonStartedTimeStamp) {
                    displayInfo = FinishedStatus;
                }
            }
            ...
            return {
                ...phase,
                timestamp: phase.timestamp === InfrastructureJob.NonStartedTimeStamp ? '' : phase.timestamp,
                duration,
                displayInfo,
    ...
    private generateHistoryPhase(name: string, phases: IInfrastructureJobHistoryPhase[]): IInfrastructureJobPhase {
    ...
    public getPhase(phase: string): IInfrastructureJobHistoryPhase {
        return this.history.find(historyPhase => historyPhase.phase === phase);
    }

    public getHistoryPhase(phase: string): IInfrastructureJobPhase {
